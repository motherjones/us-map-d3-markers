...
PremadeVis.map.prototype.state_abbr_dictionary = {
    'AL' : 'Alabama',
    'AK' : 'Alaska',
    'AS' : 'America Samoa',
    'AZ' : 'Arizona',
...
PremadeVis.map.prototype.get_xy = function(node) {
    if (node.lon && node.lat ) { 
        ...
        if (node.state.length === 2) {
            node.state = this.state_abbr_dictionary[node.state];
        }
        var state_name = node.state.toLowerCase().replace(/\s+/g, '');
        var path = jQuery('#' + state_name).attr('d');
        //FIXME REMOVE THIS WHEN WE GET A BETTER FEATURE COLLECTION
